...
import org.jasig.cas.zk.asyn.AsynProcessor;
import org.jasig.cas.zk.asyn.LastLoginTimeProcessor;
...
public abstract class AbstractServiceValidateController extends AbstractDelegateController {
    ...
    private ModelAndView generateSuccessView(final Assertion assertion, final String proxyIou,
        ...
        if(registeredService!=null){
        	//最后登录时间
        	AsynProcessor.asynProcess(new LastLoginTimeProcessor(ClientInfoEnum.getInstance((int)registeredService.getId()).getType(),
        			Integer.parseInt(assertion.getPrimaryAuthentication().getPrincipal().getAttributes().get("userId").toString()), userCenterService));
        }else{
        	logger.info("未注册的服务:"+service.getId());
        }
