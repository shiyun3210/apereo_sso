...
public class AuthenticationViaFormAction {
    ...
    public final Event submit(final RequestContext context, final Credential credential,
        ...
        Credential clientIdCredential = null;
        if(credential instanceof UsernamePasswordCredential){
        	UsernamePasswordCredential reqUserInfo = (UsernamePasswordCredential)credential;
        	clientIdCredential = new UsernamePasswordCredential(reqUserInfo.getUsername(), reqUserInfo.getPassword(), (int)registeredService.getId());
        }
        if (isRequestAskingForServiceTicket(context)) {
            return grantServiceTicket(context, clientIdCredential,service);
        }

        return createTicketGrantingTicket(context, clientIdCredential, messageContext,service);
    }
...
//        return StringUtils.equals(loginTicketFromFlowScope, loginTicketFromRequest);
        return redisLoginTicketService.isValid(context.getRequestParameters().get("lt"));
    }
     ...
     * @since 4.1.0
     */
    protected Event grantServiceTicket(final RequestContext context, final Credential credential,Service service) {
        final String ticketGrantingTicketId = WebUtils.getTicketGrantingTicketId(context);
        try {
//            final Service service = WebUtils.getService(context);
    ...
    protected Event createTicketGrantingTicket(final RequestContext context, final Credential credential,
                                               final MessageContext messageContext,final Service service) {
        try {
//            final Service service = WebUtils.getService(context);
//            String clientId = context.getRequestParameters().get("client_id").toString();
//            if(!StringUtils.isNumeric(clientId)||ClientInfoEnum.getInstance(Integer.parseInt(clientId))==null){
//            	logger.info("无效的client_id");
